...
read_result(Client = #tclient{protocol = Proto0,
    ...
    lager:info("read result messagebegin ~p", [MessageBegin]),
    case MessageBegin of
        #protocol_message_begin{seqid = RetSeqId} when RetSeqId =:= -1 ->
            read_result(Client, Function, ReplyType);
