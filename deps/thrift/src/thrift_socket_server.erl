...
parse_options([{transport, Transport} | Rest], State) ->
    parse_options(Rest, State#thrift_socket_server{transport=Transport}).
...
init(State=#thrift_socket_server{ip=Ip, port=Port}) ->
    ...
    case gen_tcp_listen(Port, Opts, State) of
        ...
        Other ->
            error_logger:info_msg("thrift service listening on port ~p~n", [Port]),
            Other
    end.
...
new_acceptor(State=#thrift_socket_server{listen=Listen,
                                         ...
                                         socket_opts=Opts, transport=Transport
                                        }) ->
    Pid = proc_lib:spawn_link(?MODULE, acceptor_loop,
                              [{self(), Listen, Service, Handler, Opts, Transport}]),
    State#thrift_socket_server{acceptor=Pid}.

acceptor_loop({Server, Listen, Service, Handler, SocketOpts, Transport})
  when is_pid(Server), is_list(SocketOpts) ->
    case catch gen_tcp:accept(Listen) of % infinite timeout
        {ok, Socket} ->
            %%record_network:record(network_accepted),
            %%error_logger:error_msg("acceptor_loop ~p ~p ~p ~p ~p ~p ",[Server, Listen, Service, Handler, SocketOpts, Transport]),
            gen_server:cast(Server, {accepted, self()}),
            SecureProtoGen = fun(AesKey, AesVector) ->
               {ok, SocketTransport}   = thrift_socket_transport_2:new(Socket, SocketOpts),
               {ok, Transport1}        = Transport:new(SocketTransport, AesKey, AesVector),
               {ok, SecureProto}       = thrift_binary_protocol:new(Transport1),
               SecureProto
            end,
            HandshakeProtoGen = fun() ->
               {ok, SocketTransport1}  = thrift_socket_transport:new(Socket, SocketOpts),
               {ok, FrameTransport}    = thrift_framed_transport:new(SocketTransport1),
               {ok, HandshakeProto}    = thrift_binary_protocol:new(FrameTransport),
               HandshakeProto
            end,
            thrift_processor:init({Server, HandshakeProtoGen, SecureProtoGen, Service, Handler});
        {error, closed} ->
            exit({error, closed});
        Other ->
